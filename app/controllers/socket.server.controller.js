...
var Router = function (eventName) {
    ...
    var Handler = function (func) {
        var params = func_params(func),
            ...
            q = params.filter(function (v) { return (v[0] === '$'); }),
            o = (function (a) {
        ...
        return {
            ...
            keys: possible,
            func: func,
            execute: function (socket, data, dontparse) {
                var k, p = [];
                if (data && !dontparse) {
    ...
    var instance = {
        ...
        addException: function (func) {
            if (this._handlers._except) {
                throw new Error('Exception handler already added for event.');
            }
            this._handlers._except = new Handler(func);
            return this;
        },
        run: function (socket, data) {
            ...
            if (!data || typeof(data) !== 'object' ||
                ...
                h = this._run_handler(this._handlers.do, socket);
            }
            else {
                h = this._handlers[Object.keys(data).sort().join(' ')];
                if (!h) {
                    h = this._run_handler(this._handlers._except, socket, data, true);
                } else {
                    h = this._run_handler(h, socket, data);
                }
            }
            if (!h) {
        ...
        _run_handler: function (h, socket, data, dontparse) {
            if (!h) return false;
            h.execute(socket, data, dontparse);
            return true;
        },
    ...
    Statics.routers[eventName] = instance;
